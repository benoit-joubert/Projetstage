...
*/
//require(XMPP_PATH."/xmlobj.php");
//require(XMPP_PATH."/logging.php");
...
class XMLStream {
    ...
    {
        $this->XMLStream($host, $port, $log, $loglevel, $is_server);
    }
	...
	function connect($persistent=False, $sendinit=True) {
		...
		$flags=$conflag;
		$this->socket = stream_socket_client("tcp://{$this->host}:{$this->port}", $flags);
		if(!$this->socket) {
		...
		if($this->xml_depth == 1) {
			...
			foreach($this->nshandlers as $handler) {
				...
				if($searchxml->name == $handler[0] and ($searchxml->ns == $handler[1] or (!$handler[1] and $searchxml->ns == $this->default_ns))) {
					...
					if (substr(str_replace('.', '', phpversion()), 0, 3) >= 550)
			...
			foreach($this->idhandlers as $id => $handler) {
				if(isset($this->xmlobj[2]->attrs['id']) && $this->xmlobj[2]->attrs['id'] == $id) {
			...
			if(isset($this->xmlobj) && is_array($this->xmlobj)) {
