...
*/
require(XMPPHP_PATH."/xmlobj.php");
require(XMPPHP_PATH."/xmlstream.php");
require(XMPPHP_PATH."/logging.php");
...
class XMPP extends XMLStream {
	...
	function __construct($host, $port, $user, $password, $resource, $server=Null, $printlog=False, $loglevel=Null)
    {
        $this->XMPP($host, $port, $user, $password, $resource, $server, $printlog, $loglevel);
    }
	...
	function message_handler($xml) {
		...
		$tmp = $xml->sub('body');
		$payload['body'] = $tmp->data;
		$tmp = $this->log;
//		$tmp->log("Message: {$xml->sub('body')->data}", LOGGING_DEBUG);
	...
	function presence_handler($xml) {
		...
		$tmp = $xml->sub('show');
		$payload['show'] = $tmp->data;
		if(!$payload['show']) $payload['show'] = $payload['type'];
		$payload['from'] = $xml->attrs['from'];
        $tmp = $xml->sub('status');
		$payload['status'] = $tmp->data;
