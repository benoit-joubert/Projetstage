...
var _functionCall = 'callSuggestions';

function initAutoComplete(form,field,functionCall,minCarac){
  _documentForm=form;
  _inputField=field;
...
var _resultCacheAction=new Object(); // mécanisme de cache des requetes
...
function mainLoop(){
  if(_oldInputFieldValue!=_currentInputFieldValue){
    ...
    var actions=_resultCacheAction[_currentInputFieldValue];
    if(suggestions)
    { // la réponse était encore dans le cache
      metsEnPlace(valeur,suggestions,actions);
    }else{
      //callSuggestions(valeur) // appel distant
      eval(_functionCall+'(valeur);');
    }
    _inputField.focus()
  }
  _oldInputFieldValue=_currentInputFieldValue;
...
var _adresseRecherche = "index.php" //l'adresse à interroger pour trouver les suggestions

function getValeur(xml, attribut, defaut)
{
    if (defaut != undefined)
        var val = defaut;
    else
...
function callSuggestionsParcelle(valeur){
  ...
  if(_xmlHttp){
    ...
    _xmlHttp.open("GET","./index.php?P=303&action=SearchParcelle&nsec="+nsec+"&q="+valeur,true);
    _xmlHttp.onreadystatechange=function() {
      if(_xmlHttp.readyState==4&&_xmlHttp.responseXML)
      {
        ...
        for (var i=0; i < items.length; ++i)
        {
            ...
            //chAction = "document.F2.suivi_dossier.value=\""+elt+"\";";
            //chAction += "window.location.href='index.php?P=6&action=ajouter&ident_resp_legal="+getValeur(items[i],'resp_legal_id')+"';";

            optionsListe.push(elt);
            optionsAction.push(chAction);
        }
        
        cacheResults(valeur,optionsListe, optionsAction);
        metsEnPlace(valeur,optionsListe, optionsAction);
      }
    };
    // envoi de la requete
    _xmlHttp.send(null)
...
function cacheResults(debut,suggestions,actions)
{
  _resultCache[debut]=suggestions;
  _resultCacheAction[debut]=actions;
}
...
//  insereCSS(".AutoCompleteDivAction",AutoCompleteDivActionStyle);
}
...
//  _completeDiv.style.borderRight="#7DC622 "+borderLeftRight+"px solid";
//  _completeDiv.style.borderLeft="#7DC622 "+borderLeftRight+"px solid";
//  _completeDiv.style.borderTop="#7DC622 "+borderTopBottom+"px solid";
//  _completeDiv.style.borderBottom="#7DC622 "+borderTopBottom+"px solid";
...
function metsEnPlace(valeur, liste, actions)
{
  ...
  for(var f=0; f<liste.length; ++f)
  {
    ...
    setStylePourElement(nouveauSpan,"AutoCompleteDivAction");
    nouveauDiv.appendChild(nouveauSpan);
    
    
    var nouveauSpan=document.createElement("SPAN");
    ch = liste[f];
    eval("ch = ch.replace(/"+valeur+"/g, \"<strong><u>"+valeur+"</u></strong>\");");
    ...
    setStylePourElement(nouveauSpanAction,"AutoCompleteDivAction");
...
var onKeyUpHandler=function(event){
  ...
  if(_eventKeycode!=0){
    ...
    if(_eventKeycode==13||_eventKeycode==3)
    {
      ...
      var actionCh = getAction(_highlightedSuggestionDiv);
      eval(actionCh);
      hideCompleteDiv();
      //alert('13 ou 3');
    } else {
...
function getAction(uneDiv){
  if(!uneDiv) {
    return null;
  }
  return trimCR(uneDiv.getElementsByTagName('span')[2].firstChild.data)
}
...
var divOnMouseDown=function()
{
  ...
  var actionCh = getAction(this);
  eval(actionCh);
  hideCompleteDiv(); // on cache le div
