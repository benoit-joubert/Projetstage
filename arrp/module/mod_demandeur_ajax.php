...
/*
	...
	$sIndexColumn = 'ID_DEMANDEUR';
	
	/* DB table to use */
	$sTable = 'ARRP_DEMANDEURS';
...
//    while ( $aRow = oci_fetch_array($statmntFinal, OCI_ASSOC) )
    while ( $aRow = oci_fetch_array($statmntFinal, OCI_RETURN_NULLS) )
    {
        ...
        if(in_array('admin',$_SESSION[PROJET_NAME]['droit']) || in_array('saisie',$_SESSION[PROJET_NAME]['droit'])){
            $row[] = '<a href="./index.php?P=301&from=2&demandeur='.$aRow[ $aColumns[0]].'"><img src="./images/distribution_eau_reseau_24.jpg" border="0" title="Ajouter une nouvelle demande"/></a>';
        }
        $row[] = '<a href="./index.php?P=201&demandeur='.$aRow[ $aColumns[0]].'&action=modifier"><img src="./images/edit_24.png" border="0" title="Voir"/></a>';
        $output['aaData'][] = $row;
    }
